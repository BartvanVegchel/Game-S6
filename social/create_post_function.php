...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Controleer of het formulier verzonden is
	...
	if (!empty($_POST['username']) &&
        ...
        $previewText	= mysqli_real_escape_string($db_social, $_POST["preview-text"]);
        ...
        $result = mysqli_query($db_social, "SELECT * FROM posts WHERE title = '$title' "); //kijken of de ingevulde title in de database staat
        ...
        if (mysqli_num_rows($result) > 0) { //checken of result meer dan 0 rijen teruggegeven, of gebruikersnaam al bestaat
            ...
            mysqli_query($db_social, "INSERT INTO posts (id, username, userId, title, previewText, content, category, postDate, image)
                                VALUES ('', '$username', '$userId', '$title', '$previewText', '$content', '$category', '$postDate', '') ");
			...
			if($_FILES['photo']['name'])
			{
				...
				if(!$_FILES['photo']['error'])
				{
					...
					if($valid_file)
					{
						...
						mysqli_query($db_social, "UPDATE posts SET image = '$imagePath' WHERE id = '$postId'");
					}
				}
